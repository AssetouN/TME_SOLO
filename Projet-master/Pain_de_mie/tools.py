...
class SuperState ( object ):
    ...
    def ball_camp(self):
        if (self.id_team==1) and (self.ball.x<=settings.GAME_WIDTH/2):
            return True
        else:
            return False
        if (self.id_team==2) and (self.ball.x>=settings.GAME_WIDTH/2):
            return True
        else:
            return False 
    ...
    def est_proche_adv(self, id_team, id_player):
        L = liste_joueur(state, id_team_adv(id_team)) 
        LP = [state.player_state(it,ip).position for (it,ip) in L if it != id_team]
        for i in range(0, len(LP)):
            if(self.state.player_state(id_team,id_player).position.distance(LP[i]) < 3):
                return true
        return false
    ...
    def joueur_proche_rang(self, rang):
        ...
        liste_position = [state.player_state(it,ip) for (it,ip) in L if ip != id_player and it == self.id_team]
...
#                 Action Joueur    
    ...
    def defense2(self):
        if self.state.ball_camp(self):
            if self.state.peut_frapper() :
                j1 = self.state.joueur_proche_rang(4)
                j2 = self.state.joueur_proche_rang(3)
                if self.state.est_proche_adv(j1[0],j1[1]):
                    return self.degagement()
                elif self.state.est_proche_adv(j2[0],j2[1]):
                    return self.degagement()
                else:
                    return passe_joueur_proche()
                
            else:
                # suivre la direction de la balle selonl'axe des y
                # si je suis prooche de la balle je m'approche de cette dernière
        else:
            #return self.aller(replacement_defense2()) et après rester immobile i think 
