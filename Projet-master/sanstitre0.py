...
class Defenseur(Strategy):
    ...
    def compute_strategy(self,state,id_team,id_player):
        fct = SuperState(state,id_team,id_player)
        if(fct.ball.distance(fct.player)>=10):
            return SoccerAction(acceleration = Vector2D(fct.player.x,fct.ball.y)-Vector2D(fct.player.x,fct.player.y),shoot = Vector2D(0,0))
        else:
            defense = Vector2D(QUARTER_WIDTH,QUARTER_HEIGHT)
            return SoccerAction(acceleration = defense- fct.player, shoot =Vector2D())
